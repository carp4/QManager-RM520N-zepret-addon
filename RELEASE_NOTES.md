...
> - **Uninstall can no longer cut your LAN web traffic.** Removing the engine now drains the REDIRECT rule with lock-wait and verification, and it stops the re-assert timer *before* the drain — under a busy iptables lock a leftover rule used to keep redirecting every client's web traffic to a dead port until the next reconnect. If the rule genuinely can't be cleared, the UI now says exactly that instead of claiming success.
