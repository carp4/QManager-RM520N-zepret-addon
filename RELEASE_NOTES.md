...
- **Configurable Latency & Loss Thresholds.** Recent Activities no longer floods with `High Latency` events on average cellular signal. A new System Settings card lets you pick `Standard / Tolerant / Very Tolerant` per row for both latency and packet loss, with a live "Current" readout next to each preset so you can pick the right one by seeing where you sit. **Default changes from 90 ms / 20 % to 250 ms / 30 %** — pick `Standard` if you want stricter thresholds.
