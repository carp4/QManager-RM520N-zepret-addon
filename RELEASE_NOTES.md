...
- **New Connection Quality settings page.** System Settings → **Connection Quality** is a dedicated page that consolidates probe sensitivity and event thresholds in one place, so you can tune how the modem watches your link without hunting through the rest of System Settings.
- **Tunable Connectivity Sensitivity.** A new **Connectivity Sensitivity** card lets you pick how aggressively the ping daemon checks if your internet is working — `Sensitive` (1 s probes, 6 s to fail, 3 s to recover) for hardwired or strong-signal setups, `Regular` for the new balanced default, `Relaxed` to match the previous QManager behavior, or `Quiet` (10 s probes) for battery- and data-conscious deployments. The card shows live probe interval / fail / recover values for the active preset and warns if the daemon hasn't applied your change after 30 seconds.
- **Configurable Latency & Loss Thresholds.** Recent Activities no longer floods with `High Latency` events on average cellular signal. The new **Connection Quality** page lets you pick `Standard / Tolerant / Very Tolerant` per row for both latency and packet loss, with a live "Current" readout next to each preset so you can pick the right one by seeing where you sit. **Default changes from 90 ms / 20 % to 250 ms / 30 %** — pick `Standard` if you want stricter thresholds.
...
- **Data Used totals and 1-second Live Traffic.** The Device Metrics card gains a new **Data Used** row showing cumulative cellular download and upload since the modem booted, sitting right above Live Traffic. Live Traffic itself now ticks once per second (up from once every two seconds), driven by a tiny dedicated daemon that reads `/proc/net/dev` directly — so the speed display feels real-time without adding any AT-command load. The active cellular interface is picked automatically (`rmnet_ipa0` preferred, `rmnet_data0` as fallback), and the totals reset cleanly when the modem subsystem restarts.
...
- **Faster, more honest ping latency.** The internet-reachability daemon is now a single static Rust binary that keeps one TCP connection open per probe target across cycles — so the latency you see on the dashboard reflects real round-trip time instead of being dominated by per-probe TCP handshake overhead (typically 50–60% lower on cellular). The daemon also detects a new `limited` connectivity state when the carrier is intercepting HTTP traffic for billing, data-cap, or activation pages, ready for an upcoming "Limited by carrier" UI badge.
...
- **Responsive Network Events page.** The page now adapts cleanly across phones, tablets, and foldables. Long event messages wrap inside the table instead of forcing the card to scroll sideways on phones, the toolbar (tabs + sort/limit/refresh) wraps to a second row when the controls outgrow the card, the page heading and description scale down on narrow viewports, and tab touch targets meet the 44 px minimum on touch devices. On desktop the card occupies a single column instead of stretching across the full width, matching the layout rhythm used elsewhere. The "Last updated" timestamp now also reflects every successful poll — empty refreshes no longer leave it stuck on the previous value.
