...
- **Tailscale SSH is now a one-toggle switch in the Tailscale Connection card.** Tailscale ships its own SSH server inside the daemon — tailnet members can SSH into this device using your Tailscale identities and admin-panel ACLs, with no changes to the device's regular SSH (dropbear) and no system-level config edits. Flipping the new "Tailscale SSH" switch enables it instantly (or queues it for the next connect if the daemon is stopped), and the preference survives the `--reset` that QManager uses to clear lingering flags on every connect — so SSH stays on through reconnects, reboots, and service restarts. Enabling prompts a confirmation dialog reminding you to review your Tailscale admin-panel ACL policy first, since access is gated entirely there. Disabling is silent.
