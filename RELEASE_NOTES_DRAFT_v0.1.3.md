...
- **Fixed Tailscale not surviving reboot** — two root causes: (1) tailscaled resets its state directory to `700` on every start, blocking the CGI from detecting the installation — `is_installed()` now checks the world-readable systemd unit file instead of traversing the restricted directory; (2) rootfs writes (unit file, boot symlink) weren't flushed before remounting read-only — added `sync` before every `mount -o remount,ro /` in the helper. Belt-and-suspenders: `ExecStartPost` in the service unit and `qmanager_setup` also restore directory permissions.
